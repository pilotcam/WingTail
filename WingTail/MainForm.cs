...
namespace WingTail
{
    public partial class MainForm : Form
    {
        ...
        {
            InitializeComponent();
            //Icon = Properties.Resources.WingIcon;
            //_trayIcon.Icon = Properties.Resources.WingIcon; 
            _instance = this;
        ...
        private string GetDefaultConfigPath()
        {
            ...
            // 1. WingTail.xml in application directory
            // 2. WingTail.xml in current user roaming app directory
            // 3. WingTail.xml in current user local app directory
            // 4. WingTail.xml in common app directory
            string appPath = Path.GetDirectoryName(Application.ExecutablePath) + "\\WingTail.xml";
            string roamingPath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\WingTail\\WingTail.xml";
            string localPath = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + "\\WingTail\\WingTail.xml";
            string commonPath = Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData) + "\\WingTail\\WingTail.xml";
        ...
        public void SaveConfig(TailConfig tailConfig, string filepath)
        {
            if (string.IsNullOrEmpty(filepath))
            {
                ...
                if (string.IsNullOrEmpty(defaultPath))
                {
                    defaultPath = Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData) + "\\WingTail\\";
                    if (!Directory.Exists(defaultPath))
                        Directory.CreateDirectory(defaultPath);

                    defaultPath += "WingTail.xml";
                }
